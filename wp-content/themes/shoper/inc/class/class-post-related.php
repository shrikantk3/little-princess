...
class shoper_Post_Related {
	...
	public function __construct() {
		...
		if( !is_admin()  )
		{
			add_action( 'shoper_site_content_type', array( $this,'site_loop_heading' ), 10 ); 
			add_action( 'shoper_site_content_type', array( $this,'site_content_type' ), 30 ); 
		}
		
		add_action( 'shoper_posts_blog_media', array( $this, 'render_thumbnail' ) ); 
		
		
		add_action( 'shoper_loop_navigation', array( $this,'site_loop_navigation' ) );
		add_action( 'shoper_single_post_navigation', array( $this,'single_post_navigation' ),10 ); 
    ...
    function site_content_type( ){
		...
		$type = apply_filters( 'shoper_content_type_filter', shoper_get_option( 'blog_loop_content_type' ) );
	...
	*/
	public function content_read_more_link( $more  ) {
		...
		return sprintf( '<div class="more-link">
		    ...
		    esc_html( shoper_get_option( 'read_more_text' ) )
        );
	...
	*/
	...
	function single_post_navigation( ) {
		...
		if( $prevPost ) :
			$html .= '<div class="col-md-6 col-sm-6"><div class="row align-items-center">';
				...
				$html .='<div class="col-md-9"><h5>'.esc_html__('Previous Article','shoper').'</h5>';
		...
		if( $nextPost ) : 
			$html .= '<div class="col-md-6 col-sm-6"><div class="row align-items-center">';
				...
				$html .='<div class="col-md-9 ml-auto align-right"><h5>'.esc_html__('Next Article','shoper').'</h5>';
		...
		$html  	 = apply_filters( 'shoper_single_post_navigation_filter', $html );
	...
	function site_loop_navigation( $type = '' ) {
		...
		if( $type == '' ){
			$type = apply_filters( 'shoper_loop_navigation_filter', get_theme_mod( 'shoper_loop_navigation', 'default' ) );
		}
		...
		if( $type == 'default' ):
			...
			the_posts_navigation(
				array(
					'prev_text' => '<span class="btn-wrap">'.esc_html__('Previous Posts', 'shoper').'</span><span class="icon"><i class="icofont-rounded-double-left"></i></span>',
					'next_text' => '<span class="btn-wrap">'.esc_html__('Next Posts', 'shoper').'</span><span class="icon"><i class="icofont-rounded-double-right"></i></span>',
					'screen_reader_text' => __('Posts navigation', 'shoper')
				)
			);
			...
			the_posts_pagination( array(
				...
				'type' => 'list',
				'mid_size' => 2,
				'prev_text' => esc_html__( 'Previous', 'shoper' ),
				'next_text' => esc_html__( 'Next', 'shoper' ),
				'screen_reader_text' => esc_html__( '&nbsp;', 'shoper' ),
			) );
		echo '</div>';
		endif;
...
$shoper_post_related = new shoper_Post_Related();